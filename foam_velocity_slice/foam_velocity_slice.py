...
# -*- coding: utf-8 -*-
"""
OpenFOAM の結果を読み込み、座標原点 (0,0,0) を通る座標平面でスライスした
流速（速度ベクトル U の大きさ）のコンタ図を画像として保存する。

平面の法線を --normal で指定する（例: z なら平面は z=0、法線は +Z）。
...
使用例:
  ...
  # --origin は OX OY OZ の3語、または "(OX, OY, OZ)" / OX,OY,OZ の1語でも可
...
class _OriginTripletAction(argparse.Action):
    """--origin に 3 数値、または (OX,OY,OZ) 形式の 1 引数を渡す。"""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="OpenFOAM 結果を読み、原点を通る平面で流速コンタを保存します。"
    )
    parser.add_argument(
        "case",
        type=Path,
    ...
    parser.add_argument(
        ...
        choices=("x", "y", "z"),
        default="z",
        help="スライス面の法線方向（x→x=0 面、y→y=0 面、z→z=0 面）",
    )
    parser.add_argument(
        "--time",
        type=str,
    ...
    parser.add_argument(
        ...
        default=(0.0, 0.0, 0.0),
        help=(
            "スライス面が通る点: OX OY OZ の3語、または "
            '"(OX, OY, OZ)" / OX,OY,OZ の1語（既定: 0 0 0）'
        ),
    )
    parser.add_argument(
        "--cmap",
    ...
    if sl.n_cells == 0:
        print(
            "警告: スライスが空です。原点付近にメッシュが無い、または面が領域と交差していません。",
            file=sys.stderr,
        )
    ...
    plotter.close()
    print(f"保存しました: {out}  (time={t}, normal={args.normal})")
    return 0
