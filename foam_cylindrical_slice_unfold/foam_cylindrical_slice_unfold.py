...
θ=±π の接縫をまたぐセルは除去してから展開する（引き伸ばしによる偽の非連続を防ぐ）。
コンター用スカラーが cell データのときは point へ平均化する。
--scalars U_r で元座標系の速度半径方向成分 U·e_r をコンターできる。
...
def _resolve_openfoam_path(case_path: Path) -> Path:
    """
    OpenFOAMReader に渡すパスを返す（.foam ファイル）。

    VTK 9.6 以降の vtkOpenFOAMReader は SetDirectoryName が無く、
    ケースディレクトリを直接渡すと PyVista が失敗するため、
    .foam が無ければ空のプレースホルダを作成してそのパスを返す。
    """
    p = case_path.resolve()
    if p.is_file():
        return p
    ...
    foam = p / f"{p.name}.foam"
    if not foam.is_file():
        foam.write_text("", encoding="utf-8")
        print(f"注意: {foam.name} が無かったため空ファイルを作成しました（VTK OpenFOAM 読込用）。")
    return foam
...
def _cells_crossing_theta_branch_cut(mesh, theta: np.ndarray) -> np.ndarray:
    """
    atan2 の分岐（θ=±π）をまたぐセルのマスクを返す。

    そうしたセルをそのまま展開すると、u が領域の両端に飛び
    画面を横断する引き伸ばし面になり、コンターが非連続に見える。
    """
    n_cells = int(mesh.n_cells)
    crossing = np.zeros(n_cells, dtype=bool)
    if n_cells == 0:
        return crossing

    def _mark(i: int, ids: np.ndarray) -> None:
        if ids.size == 0:
            return
        th = theta[ids]
        if float(np.max(th) - np.min(th)) > np.pi:
            crossing[i] = True

    conn = getattr(mesh, "cell_connectivity", None)
    offsets = getattr(mesh, "offset", None)
    if conn is None or offsets is None:
        conn = getattr(mesh, "_connectivity_array", None)
        offsets = getattr(mesh, "_offset_array", None)

    if conn is not None and offsets is not None:
        conn_a = np.asarray(conn, dtype=np.int64)
        off_a = np.asarray(offsets, dtype=np.int64)
        for i in range(n_cells):
            _mark(i, conn_a[off_a[i] : off_a[i + 1]])
        return crossing
    ...
    if faces is not None and len(faces):
        ...
        i = 0
        ci = 0
        while i < faces_a.size and ci < n_cells:
            n = int(faces_a[i])
            _mark(ci, faces_a[i + 1 : i + 1 + n])
            i += n + 1
            ci += 1
        return crossing

    for i in range(n_cells):
        _mark(i, np.asarray(mesh.get_cell(i).point_ids, dtype=np.int64))
    return crossing


def _remove_theta_seam_cells(mesh, axis: str, axis_origin: tuple[float, float, float]):
    """θ 接縫をまたぐセルを除去したメッシュと、除去数を返す。"""
    pts = np.asarray(mesh.points, dtype=float)
    _, theta, _ = _cylindrical_rt_axial(pts, axis, axis_origin)
    crossing = _cells_crossing_theta_branch_cut(mesh, theta)
    n_cross = int(np.count_nonzero(crossing))
    if n_cross == 0:
        return mesh, 0
    cleaned = mesh.remove_cells(np.nonzero(crossing)[0], inplace=False)
    return cleaned, n_cross


def _ensure_point_scalar(mesh, name: str):
    """コンター用スカラーを point データにする（cell のみなら平均化）。"""
...
def _screenshot_surface_lic(
    ...
    *,
    ...
    if not np.isfinite(vectors[:, :2]).any() or np.allclose(vectors[:, :2], 0.0):
        raise RuntimeError(
...
def _cylinder_e_radial(
    points: np.ndarray,
    axis: str,
    origin: tuple[float, float, float],
) -> np.ndarray:
    """
    元座標系での円筒半径方向単位ベクトル e_r（外向き）。(N, 3)。
    """
    ox, oy, oz = origin
    dx = points[:, 0] - ox
    dy = points[:, 1] - oy
    ...
    if ax == "z":
        r = np.sqrt(dx * dx + dy * dy)
        e_r = np.column_stack([dx, dy, np.zeros(n, dtype=points.dtype)])
    elif ax == "y":
        r = np.sqrt(dx * dx + dz * dz)
        e_r = np.column_stack([dx, np.zeros(n, dtype=points.dtype), dz])
    elif ax == "x":
        r = np.sqrt(dy * dy + dz * dz)
        e_r = np.column_stack([np.zeros(n, dtype=points.dtype), dy, dz])
    else:
        raise ValueError(f"axis は x, y, z のいずれか: {axis!r}")
    r_safe = np.maximum(r, 1e-30)
    return e_r / r_safe[:, np.newaxis]


def _velocity_radial_component(
    points: np.ndarray,
    U: np.ndarray,
    axis: str,
    axis_origin: tuple[float, float, float],
) -> np.ndarray:
    """世界座標の U の半径方向成分 U·e_r（展開前の円筒基底）。"""
    if U.ndim != 2 or U.shape[1] < 3:
        raise RuntimeError(f"U の形状が想定外です: {U.shape}（(N,3) を想定）")
    e_r = _cylinder_e_radial(points, axis, axis_origin)
    return np.einsum("ij,ij->i", U[:, :3], e_r)


def _is_radial_scalar_alias(name: str | None) -> bool:
    if name is None:
        return False
    return name.strip().lower() in ("u_r", "ur", "u_radial")


def _add_velocity_radial_scalar(
    mesh,
    points: np.ndarray,
    axis: str,
    axis_origin: tuple[float, float, float],
    ...
    """U·e_r を point スカラーとして追加し、配列名を返す。"""
    u_pts = _ensure_point_velocity(mesh)
    if u_pts is None:
        raise RuntimeError(
            f"{name}（半径方向成分）には速度場 U が必要ですが見つかりません。"
        )
    if u_pts.shape[0] != points.shape[0]:
        raise RuntimeError(
            f"U の点数 ({u_pts.shape[0]}) が頂点数 ({points.shape[0]}) と一致しません。"
        )
    mesh.point_data[name] = _velocity_radial_component(
        points, u_pts, axis, axis_origin
    )
    return name
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "塗り分けに使うスカラー名（未指定時は U から |U|、なければ先頭の1成分スカラー）。"
            "特殊名 U_r / Ur / U_radial は元座標系での速度半径方向成分 U·e_r"
        ),
    )
    parser.add_argument(
        "-o",
    ...
    if r0 <= 0:
        ...
        return 1
    if args.surface_lic and args.no_u_unfold:
        print(
            "エラー: --surface-lic は U_unfold が必要なため --no-u-unfold と併用できません。",
            file=sys.stderr,
        )
        return 1
    ...
    cut, n_seam = _remove_theta_seam_cells(cut, args.axis, axis_origin)
    if n_seam:
        print(
            f"θ=±π 接縫をまたぐセルを {n_seam} 個除去しました（展開時の引き伸ばし防止）。"
        )
    if cut.n_cells == 0:
        print(
            "エラー: 接縫セル除去後にメッシュが空になりました。",
            file=sys.stderr,
        )
        return 1
    ...
    try:
        if _is_radial_scalar_alias(args.scalars):
            scalar_name = _add_velocity_radial_scalar(
                cut, pts, args.axis, axis_origin, name="U_r"
            )
            print("コンター用スカラー U_r（元座標系の半径方向成分 U·e_r）を追加しました。")
        else:
            scalar_name = _pick_scalar_array_name(cut, args.scalars)
            cut = _ensure_point_scalar(cut, scalar_name)
    ...
    if args.surface_lic and not have_u_unfold:
        print(
            "エラー: --surface-lic には速度 U から作る "
            f"{name_u!r} が必要ですが、追加できませんでした。",
            file=sys.stderr,
        )
        return 1
